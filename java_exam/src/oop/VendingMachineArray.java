...
public class VendingMachineArray {
	...
	public int orderVendingMachine(int menu, int quantity) {
		...
		// 주문 가능 수량을 넘은 경우
		if(drink.stockInQuantity < quantity) {
			System.out.println("재고가 부족합니다. 수량을 다시 선택해주세요.");
			return 0;
		}
		
		//재고에서 판매수량 빼기
		drink.stockInQuantity -= quantity;
		System.out.println(drink.name + " 음료를 " + quantity + "개 주문 했습니다.");
	...
	public void stockInVendingMachine(int menu, int stockInQuantity) {
		...
		VendingMachine drink = this.drinks[menu];
		drink.stockInQuantity += stockInQuantity;
		System.out.println(drink.name + " 의 음료가" + stockInQuantity +"개 만큼 추가입고되었습니다.");
	}
	...
	public void printAllStockInQuantity() {
		...
		for(int i = 0; i < this.drinks.length; i++) {
			System.out.println(this.drinks[i].name + " " + this.drinks[i].stockInQuantity+"개 남았습니다.");
		}
	}
}
