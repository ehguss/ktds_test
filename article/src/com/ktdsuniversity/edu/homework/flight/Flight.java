...
public class Flight {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < classes.size(); i++) {
			isCanReserve = classes.get(i).get(i-1).isCanReserve();
			if(isCanReserve) {
				return;
			}
		}
		...
		System.out.println("\"" + seatNum + "\"번 좌석을 예약하시겠습니까? (y/N): ");
		String isConfirmed = sc.nextLine();
		
		if(isConfirmed.toLowerCase() == "n") {
